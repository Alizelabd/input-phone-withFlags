...
dataList.map((e, i) => {
    ...
    <span style="display: inline-block; width: 30px; height: 22px;" class="fi fi-${e.code.toLowerCase()}"></span> ${e.name} ${e.dial_code}
    </li>
    `
});
...
liArray.forEach((li) => {
    li.addEventListener(("click"), (e) => {
        ...
        liEl.innerHTML = `
        <span style="display: inline-block; width: 30px; height: 22px;" class="fi fi-${e.target.dataset.code.toLowerCase()}"></span>
        <span>${e.target.dataset.num}</span>
        `;
    });
});
